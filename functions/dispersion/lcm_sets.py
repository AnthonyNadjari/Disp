"""
LCM parameter sets — dispersion policy (portal-free, unit-testable).
...
Policy
------
* ``lambda_pricing`` and ``lambda_atm`` default to the **EqEq lambda** of the
  run (the same number that is ``ACEqEqSpread`` in the model context). An
